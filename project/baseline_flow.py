...
labeling_function = lambda row: int(row["rating"] > 4.0)
...
class BaselineNLPFlow(FlowSpec):
    ...
    def baseline(self):
        ...
        mode = self.traindf["label"].mode()[0]
        self.baseline_predictions = np.full(len(self.valdf), mode)
        ...
        print(confusion_matrix(self.valdf.label, self.baseline_predictions))
    ...
    def end(self):
        ...
        self.false_pos = self.valdf[(self.valdf['label'] == 0) & (self.baseline_predictions == 1)]
        if len(self.false_pos) > 0:
          current.card.append(Table.from_dataframe(self.false_pos[['review']].sample(5)))
        
        current.card.append(Markdown("## Examples of False Negatives"))
        # TODO: compute the false positive predictions where the baseline is 0 and the valdf label is 1. 
        # TODO: display the false_negatives dataframe using metaflow.cards
        self.false_neg = self.valdf[(self.valdf['label'] == 1) & (self.baseline_predictions == 0)]
        if len(self.false_neg) > 0:
